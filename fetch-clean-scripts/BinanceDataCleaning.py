...
def get_csv(pair: str):
    ...
    :return: CSV Stream
    """
    return open(f"data/{pair}_Binance5MCandles.csv")
...
def clean_data(pair):
    ...
    :param pair: Pair for which to clean the data
    """
    try:
        csv = get_csv(pair)
    ...
    # Drop unused columns
    dirty = dirty.drop(
        columns=["Trash1", "Trash2", "Trash3", "Trash4", "Trash5", "Trash6"]
    )
